(function(){
  ...
  function initParticles(){
    particles.length=0;
    for(let i=0;i<N;i++){
      particles.push({x:rand(0,w),y:rand(0,h),vx:rand(-0.15,0.15),vy:rand(-0.15,0.15),r:rand(0.6,2.2),t:Math.random()*Math.PI*2});
    }
  }
  ...
  function step(){
    ...
    for(const p of particles){
      p.x+=p.vx; p.y+=p.vy; p.vx*=0.995; p.vy*=0.995; p.t+=0.002;
